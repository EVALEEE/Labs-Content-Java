...
//                System.out.println(animals[x][y]);
                this.hunger++;
                this.randomMove();
            }
        }
    ...
    class Fox extends Animal {
        ...
        void moveAndEat() {
            // FIXME complete this method
            List<Animal> neighborRabbit = new ArrayList<>();
            int leftX = (x - 1 + World.this.sizeX) % World.this.sizeX;
            int rightX = (x + 1) % World.this.sizeX;
            int upY = (y - 1 + World.this.sizeY) % World.this.sizeY;
            int downY = (y + 1) % World.this.sizeY;

            if (animals[leftX][y] instanceof Rabbit) {
                neighborRabbit.add(animals[leftX][y]);
            }
            if (animals[rightX][y] instanceof Rabbit) {
                neighborRabbit.add(animals[rightX][y]);
            }
            if (animals[x][upY] instanceof Rabbit) {
                neighborRabbit.add(animals[x][upY]);
            }
            if (animals[x][downY] instanceof Rabbit) {
                neighborRabbit.add(animals[x][downY]);
            }
            if (animals[leftX][upY] instanceof Rabbit) {
                neighborRabbit.add(animals[leftX][upY]);
            }
            if (animals[rightX][upY] instanceof Rabbit) {
                neighborRabbit.add(animals[rightX][upY]);
            }
            if (animals[leftX][downY] instanceof Rabbit) {
                neighborRabbit.add(animals[leftX][downY]);
            }
            if (animals[rightX][downY] instanceof Rabbit) {
                neighborRabbit.add(animals[rightX][downY]);
            }

            if (neighborRabbit.isEmpty()) {
                ...
                if (neighborRabbit.size() == 1) {
                    Animal selectedRabbit = neighborRabbit.get(0);
                    animals[selectedRabbit.x][selectedRabbit.y] = this;
                    ...
                    int randomIndex = random.nextInt(neighborRabbit.size());
                    Animal selectedRabbit = neighborRabbit.get(randomIndex);
                    animals[selectedRabbit.x][selectedRabbit.y] = this;
    ...
    /**
     * This method is only used for testing purposes, although you are free to use
     * it if you need.
     *
     * @param x the x-coordinate at which the cell is located
     * @param y the y-coordinate at which the cell is located
     * @return the amount of grass at (x, y)
     */
     ...
     * @param x the x-coordinate at which the cell is located
     * @param y the y-coordinate at which the cell is located
     * @return the animal at (x, y), or `null` if no animal is at that cell
     */
        ...
        for (int col = 0; col < sizeX; col++) {
            for (int row = 0; row < sizeY; row++) {
                if (animals[col][row] == null) {
                    state = state + grass[col][row] + "  ";
                } else if (animals[col][row] instanceof Rabbit) {
                    state = state + grass[col][row] + "r" + animals[col][row].hunger;
                } else if (animals[col][row] instanceof Fox) {
                    state = state + grass[col][row] + "f" + animals[col][row].hunger;
                }
            }
        }
        return state;
