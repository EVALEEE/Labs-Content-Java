# COMP1110 Week 5 Lab
<!--
Before you start, you may wish to take a minute to watch the [intro video](https://comp.anu.edu.au/courses/comp1110/labs/mp4/lab5-intro.mp4) that runs through the goals of this lab.
-->
This lab contains material from modules [**S2**](https://comp.anu.edu.au/courses/comp1110/lectures/theme/#S2) (Branches and Merging) and [**J4**](https://comp.anu.edu.au/courses/comp1110/lectures/theme/#J4) (Random).

We have added additional "issues" for week 5. However, they are not transferred to Gitlab by an upstream pull.
Instead, find the file `comp1110-labs-issues-week5.csv` after doing an upstream pull.
...
* Watch the [lab demo video](https://comp.anu.edu.au/courses/comp1110/labs/mp4/lab5-demo.mp4) for this lab. This is a short video introducing some key concepts in this lab.
...
When you're not checking in with your tutor, watch the [lab intro video](https://comp.anu.edu.au/courses/comp1110/labs/mp4/lab5-intro.mp4) and start on Part II below.

## Part II:  Your Lab Tasks

### 1. GitLab Upstream Pulls

If you have not already done so, try an upstream pull of the labs and homework repositories to ensure that you have the latest version.
Refer to per the notes for [lab 2](../lab2/README.md#gitlab-pull-upstream-commits) and the [guide on the course website](https://comp.anu.edu.au/courses/comp1110/help/upstream-pull/).

You should also regularly try upstream pulls for assignment 2, in case there have been any changes (this week, we will release an update at the end of the week).
For assignment 2, *one* member of the group should pull changes from upstream, then push the changes to your group's assignment repository.
...
This exercise is demonstrated in a [how-to video](https://comp.anu.edu.au/courses/comp1110/help/mp4/gitexercise.mp4).
If you have not seen the video, we strongly recommend you watch it.
If you get into trouble doing this exercise, you may find it helpful to look at the video again.
In the [video](https://comp.anu.edu.au/courses/comp1110/help/mp4/gitexercise.mp4), Steve Blackburn performed the role of all three characters (Alice, Bob and Cindy).
...
2.  **Alice** does the following (with the others watching...):
    ...
    * You should see a notification dialogue at the bottom of the workspace "Pushed 1 commit to origin/main", which indicates that the push was successful.
...
8. **Everyone** does the following (*at the same time*):
    ...
    * Ensure that you are on the head of the `main` branch (i.e. not on Bob's experimental branch).
      ...
      Bob will be on the wrong changeset, so he should switch to the `main` branch: select **Git > Branches...**, then **Local Branches > main**, then **Checkout**.
...
where we introduced an error;
`output[inner_index] = repeatInt(current, sequence[outer_index + 1]);`
in this case we had the inner and outer indexes in the incorrect
